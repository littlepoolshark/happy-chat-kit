...
import type { HappyMessage, HappyReactMarkdownComponentProps } from '../../core/types';
...
const defaultAvatarStyle = { width: 42, height: 42, borderRadius: '50%', backgroundColor: '#e8e8e8' };

interface HappyChatMessageFlowProps extends Pick<HappyReactMarkdownComponentProps, 'isRequesting' | 'request2Agent'> {
...
function HappyChatMessageFlow({
  ...
  const visibleMessages = messages.filter((item) => {
    if (typeof item.message === 'string') return !hiddenMessages.includes(item.message);
    return true;
  });
  ...
  const bubbleItems: GetProp<typeof Bubble.List, 'items'> = visibleMessages.map(({ id, message, status }, index) => {
    ...
    if (isAnswer) {
      ...
      footer = <ActionBar position="question" text2Copy={content || ''} onEdit={() => {}} />;
    }

    const isLoading =
      status === 'loading' && (content === '' || content.startsWith(HEARTBEAT_DIRECTIVE));
    ...
    return {
      ...
      content: isQuestion ? (
        <Flex vertical gap={4}>
          ...
          <div className="user-message-footer" style={{ display: 'flex', justifyContent: 'flex-end' }}>{footer}</div>
        </Flex>
      ) : (
        <Flex vertical gap={0}>
          {agentTitle != null && (
            <div style={{ color: 'rgba(0,0,0,0.88)', fontSize: 16, fontWeight: 600, marginBottom: 8 }}>
              {agentTitle}
            </div>
          )}
          <MarkdownRenderer content={content || ''} isRequesting={isRequesting} request2Agent={request2Agent} />
          {footer}
        </Flex>
      ),
    };
